...
import { parseUnifiedDiff, type Hunk } from './parseUnifiedDiff';
...
export class DiffController implements vscode.Disposable {
  ...
  private syncSubscriptions(): void {
    ...
    for (const s of this.sessions.list()) {
      if (!this.eventSubs.has(s.id)) {
        this.eventSubs.set(
          s.id,
          s.onEvent((e) => this.onEvent(s.id, e)),
        );
      }
    }
  ...
  private async snapshotOnStart(sessionId: string, e: T.ToolExecutionStartEvent): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        this.snapshotTimes.set(filePath, Date.now());
      }
  ...
  private async presentOnEnd(sessionId: string, e: T.ToolExecutionEndEvent): Promise<void> {
    ...
    const snapshot =
      this.snapshotProvider.provideTextDocumentContent(this.snapshotUri(sessionId, filePath)) ?? '';
    if (snapshot === (await this.readCurrent(filePath))) {
      this.perFileBusy.delete(filePath);
      return;
    }
    ...
    this.current = {
      ...
      acceptedHunkIndices: new Set(),
      rejectedHunkIndices: new Set(),
    };
  ...
  private async openDiffEditor(p: PendingDiff): Promise<void> {
    ...
    await vscode.commands.executeCommand(
      ...
      left,
      right,
      `Pi edit — ${path.basename(p.filePath)}`,
      { preview: false },
    );
    if (p.firstChangedLine !== undefined && p.firstChangedLine >= 0)
      this.revealChangedLine(p.filePath, p.firstChangedLine);
  ...
  private async applyRejectedHunks(p: PendingDiff): Promise<void> {
    ...
    try {
      ...
      if (changed) {
        ...
        this._ctx.log(
          'info',
          `Applied ${p.rejectedHunkIndices!.size} rejected hunk(s) to ${path.basename(p.filePath)}`,
        );
      }
    } catch (err) {
  ...
  private async detectConflict(filePath: string): Promise<string | null> {
    ...
    try {
      ...
      return `File ${path.basename(filePath)} no longer exists. Cannot revert.`;
    }
    return null;
  }
  ...
  private applyDecoration(
    ...
    dt: vscode.TextEditorDecorationType,
    ranges: vscode.Range[],
  ): void {
  ...
  private snapshotUri(sessionId: string, filePath: string): vscode.Uri {
    return vscode.Uri.parse(
      `${PI_DIFF_SCHEME}://pi/${encodeURIComponent(sessionId + '|' + filePath)}`,
    );
  }
  ...
  async acceptCurrent(): Promise<void> {
    if (this.current) {
      this.clearDecoration(this.current.filePath);
      this.perFileBusy.delete(this.current.filePath);
      this.current = null;
    }
  ...
  async revertCurrent(): Promise<void> {
    const p = this.current;
    if (!p) {
      vscode.window.showWarningMessage('No Pi edit to revert.');
      return;
    }
    ...
    ws.replace(
      ...
      p.snapshot,
      { needsConfirmation: false, label: 'Pi revert' } as any,
    );
    const ok = await vscode.workspace.applyEdit(ws);
    if (!ok) {
      vscode.window.showErrorMessage(`Failed to revert ${p.filePath}.`);
      return;
    }
    ...
    if (session && session.state?.isStreaming)
      void session.steer(
        `I reverted your edit to ${path.basename(p.filePath)}. Please try a different approach.`,
      );
