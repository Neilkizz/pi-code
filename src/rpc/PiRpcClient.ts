...
export class PiRpcClient implements vscode.Disposable {
	...
	private scheduleRestart(): void {
		...
		if (this.restartCount > PiRpcClient.MAX_RESTART_COUNT) {
			...
			this.log(
				"error",
				`reached max restart count (${PiRpcClient.MAX_RESTART_COUNT}) — giving up`,
			);
			this.restarting = false;
	...
	private onLine(line: string): void {
		...
		if (obj.type === "response") {
			...
			try {
				...
				this.log(
					"error",
					`event handler threw for ${obj.type}: ${(err as Error).message}`,
				);
			}
		}
	}
		...
		return new Promise<R>((resolve, reject) => {
			const timer = setTimeout(() => {
				if (this.pending.has(id)) {
					...
					reject(
						new Error(
							`Request ${command} timed out after ${this.opts.requestTimeoutMs}ms`,
						),
					);
				}
