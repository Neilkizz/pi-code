...
export function MessageItem({ msg, onPlanAction }: Props) {
  ...
  return (
    <div className={`message ${msg.kind}${streamingClass}`}>
      {msg.kind === 'assistant' && msg.thinkingText && <ThinkingBlock text={msg.thinkingText} />}
...
function ToolCallBlock({ msg }: { msg: DisplayMessage }) {
  ...
  const statusDot =
    msg.toolStatus === 'running' ? 'blue' : msg.toolStatus === 'done' ? 'green' : 'red';
...
function InlineMarkdown({ text }: { text: string }) {
  ...
  lines.forEach((line, li) => {
    ...
    if (ulMatch) {
      ...
      inList.push(
        <li key={`li-${li}`}>
          <span className="bullet-dot grey">●</span> <InlineMarkdownInline text={ulMatch[2]} />
        </li>,
      );
      return;
    }
