...
function handleWorkspaceTrust(ctx: ExtensionContext): vscode.Disposable {
  const update = () => {
    ...
    void vscode.commands.executeCommand('setContext', 'pi.workspaceTrusted', trusted);
    if (!trusted) {
      ctx.log('warn', 'Workspace is NOT trusted — Pi Code running in read-only mode');
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  context.subscriptions.push(vscode.window.registerTreeDataProvider('piSessions', sessionTree));
  ...
  disposables.push(...registerCommands(ctx, sessions, chat, auth, diff, terminal));
