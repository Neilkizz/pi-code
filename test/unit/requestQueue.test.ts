...
describe("RequestQueue", () => {
  it("enqueues and drains in FIFO order", () => {
    ...
    const order: string[] = [];
    q.enqueue({
      id: "a", command: "prompt", payload: "", resolve: () => order.push("a"), reject: () => {}, timeoutMs: 5000,
    });
    q.enqueue({
      id: "b", command: "prompt", payload: "", resolve: () => order.push("b"), reject: () => {}, timeoutMs: 5000,
    });
  ...
  it("clear rejects all items", () => {
    ...
    q.enqueue({
      id: "1", command: "prompt", payload: "",
      resolve: () => {}, reject: (e: Error) => rejected.push(e.message), timeoutMs: 5000,
    });
  ...
  it("remove drops a specific request", () => {
    const q = new RequestQueue();
    q.enqueue({
      id: "x", command: "prompt", payload: "",
      resolve: () => {}, reject: () => {}, timeoutMs: 5000,
    });
    q.enqueue({
      id: "y", command: "prompt", payload: "",
      resolve: () => {}, reject: () => {}, timeoutMs: 5000,
    });
    q.remove("x");
  ...
  it("drops oldest when max size exceeded (default 100)", () => {
    ...
    q.enqueue({
      id: "1st", command: "", payload: "",
      resolve: () => {}, reject: () => dropped.push("1st"),
      timeoutMs: 5000,
    });
    q.enqueue({
      id: "2nd", command: "", payload: "",
      resolve: () => {}, reject: () => dropped.push("2nd"),
      timeoutMs: 5000,
    });
    q.enqueue({
      id: "3rd", command: "", payload: "",
      resolve: () => {}, reject: () => dropped.push("3rd"),
      timeoutMs: 5000,
    });
    // Full — push a 4th item, 1st should be dropped.
    q.enqueue({
      id: "4th", command: "", payload: "",
      resolve: () => {}, reject: () => dropped.push("4th"),
      timeoutMs: 5000,
    });
