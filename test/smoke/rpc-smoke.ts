...
class JsonlReader {
  ...
  parsed(): any[] {
    return this.lines.map((l) => {
      try { return JSON.parse(l); } catch { return { type: "unparseable", raw: l.slice(0, 80) }; }
    });
  }
...
async function run(): Promise<void> {
  ...
  child.stdin!.write(JSON.stringify({ id: promptId, type: "prompt", message: "Say hello in one sentence." }) + "\n");
  ...
  await new Promise<void>((resolve) => {
    ...
    const interval = setInterval(() => {
      const found = freshReader.parsed().find((e: any) => e.type === "response" && e.id === stateId);
      if (found) {
        clearInterval(interval);
        clearTimeout(t);
