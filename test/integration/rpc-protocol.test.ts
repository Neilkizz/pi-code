...
describe('PiRpcClient protocol (integration)', () => {
  ...
  it('survives a JSON response split across multiple write() calls (chunks scenario)', async () => {
    // The chunks scenario writes ~60% of the response, waits, writes ~30% more,
    // waits, then writes the final ~30% + newline. The client should reassemble
    // and parse the full JSON object without error.
