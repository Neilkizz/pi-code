...
| | F-004 | 多位置显示 | ⚠️ | 仅主侧栏 (WebviewView), 不支持辅助侧栏/标签页 |
...
| **4. 计划模式** | | | | |
| | F-301~F-307 | 计划生成/审批/变更 | ❌ | `pi` CLI 在 RPC 模式下不暴露计划阶段 |
| **5. 文件修改与 Diff** | | | | |
...
| | F-405 | 用户二次编辑 | ❌ | pi CLI 控制的执行通道, 扩展无法截取 |
...
| | F-705~F-711 | 检查点/回滚 | ❌ | `pi` CLI 控制会话持久化 |
| **10. Git 能力** | | | | |
...
| | Agent Core ≥90% | ❌ | Agent Core 委托给 `pi` CLI |
...
- 计划模式 (F-301~F-307)
- 检查点/回滚 (F-705~F-711)
- Provider 适配 (Section 4)
- MCP 管理 (Section 5.1)
- 子智能体 (Section 5.4)
- Git 写操作 (Section 3.10)
- 流式状态机验证 (T-001~T-012)
- 大多数 E2E 场景 (需 Extension Development Host)
...
| 标准 | 需要的能力 |
|------|-----------|
| F-301~F-307 计划模式 | pi CLI RPC 需暴露 `plan` 命令和 readonly 阶段 |
| Section 4 Provider 适配 | pi CLI 需暴露 Provider 注册、切换和事件标准化 |
| Section 5.1 MCP 管理 | pi CLI 需暴露 MCP 服务器增删和状态 |
| Section 5.4 子智能体 | pi CLI 需暴露子智能体调用和结果 |
| F-705~F-711 检查点/回滚 | pi CLI 需暴露 `checkpoint`/`rollback` RPC |
| Section 3.10 Git 写操作 | pi CLI 需暴露 git 操作结果供审计 |
| T-001~T-012 流式状态机 | pi CLI 需实现并暴露状态验证事件 |
| Section 4.4 故障转移 | pi CLI 需暴露故障转移事件和备用模型选择 |
| F-405 用户二次编辑 | pi CLI 需暴露编辑前候选内容查询 |
| E2E Scenario 8/9/10/18/19 | 需要 pi CLI 协议变更 + Extension Dev Host |
